...
class HttpCall<V> {
  ...
  static class Factory {
    ...
    Factory(OkHttpClient ok, HttpUrl baseUrl) {
      this.ok = ok;
      this.baseUrl = baseUrl;
    }

    <V> HttpCall<V> newCall(Request request, BodyConverter<V> bodyConverter) {
      return new HttpCall(this, request, bodyConverter);
    }

    <V> V execute(Request request, BodyConverter<V> bodyConverter) {
  ...
  private HttpCall(Factory factory, Request request, BodyConverter<V> bodyConverter) {
    this.ok = factory.ok;
    this.request = request;
    this.bodyConverter = bodyConverter;
  }

  void submit(Callback<V> delegate) {
