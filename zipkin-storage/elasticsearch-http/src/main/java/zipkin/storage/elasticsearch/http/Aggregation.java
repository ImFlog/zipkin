...
class Aggregation {
  ...
  private Aggregation(String field) {
    this.field = field;
  }

  static Aggregation nestedTerms(String field) {
  ...
  static Aggregation terms(String field, int size) {
    Aggregation result = new Aggregation(field);
    result.terms = new AggTerms(field, size);
    return result;
  }

  Aggregation orderBy(String subAgg, String direction) {
